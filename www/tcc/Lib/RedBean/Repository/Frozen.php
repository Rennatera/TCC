...
namespace Lib\RedBean\Repository;

use Lib\RedBean\OODBBean as OODBBean;
use Lib\RedBean\QueryWriter as QueryWriter;
use Lib\RedBean\RedException as RedException;
use Lib\RedBean\BeanHelper as BeanHelper;
use Lib\RedBean\RedException\SQL as SQLException;
use Lib\RedBean\Repository as Repository;
...
class Frozen extends Repository
{
	...
	{
		$OODBBEAN = defined( 'REDBEAN_OODBBEAN_CLASS' ) ? REDBEAN_OODBBEAN_CLASS : '\Lib\RedBean\OODBBean';
		$beans = array();
		for ( $i = 0; $i < $number; $i++ ) {
			/** @var \Lib\RedBean\OODBBean $bean */
			$bean = new $OODBBEAN;
